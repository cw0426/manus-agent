...
const navigateTo = (path) => {
  // 从主页进入时，清除保存的会话ID，触发创建新对话
  if (path === '/love-master') {
    localStorage.removeItem('ai_chat_love_current_session')
  } else if (path === '/super-agent') {
    localStorage.removeItem('ai_chat_super_current_session')
  }
  router.push(path)
}
</script>
